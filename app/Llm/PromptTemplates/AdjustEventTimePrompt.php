...
class AdjustEventTimePrompt extends AbstractLlmPromptTemplate
{
    public function systemPrompt(): string
    {
        return 'You are an event scheduling assistant helping a student reschedule around classes and commitments. Goal: suggest new start/end times when the user asks to move, reschedule, or shift an event. '
            ...
            .'Return a single JSON object with: entity_type (exactly "event"), recommended_action (1–3 sentences: the new time, in a warm tone), reasoning (2–4 sentences: why this slot works—reference the Context, e.g. "your afternoon is free", "this fits after your class"—and, if natural, one short encouraging sentence). '
            ...
            .'When you suggest a new time, always include start_datetime and end_datetime (ISO 8601) and the same in proposed_properties. Optionally confidence (0–1), timezone, location. Do not list step numbers in reasoning. When user_scheduling_request in Context already contains an explicit date and/or time (e.g. "Friday at 9am"), treat that slot as the student’s choice: do NOT say you "chose" or "moved it to" that time— instead, explain why their requested slot works (for example, "since you want it at 9am on Friday and your morning is free…"). Only propose a different time when the requested slot is impossible, and in that case clearly explain why and ask for a new choice, never silently changing it. '
