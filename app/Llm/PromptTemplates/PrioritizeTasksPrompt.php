...
class PrioritizeTasksPrompt extends AbstractLlmPromptTemplate
{
    ...
    {
        return 'You are a task prioritization expert helping a student decide what to work on first. Goal: rank tasks by true urgency and impact (deadlines, dependencies, effort vs impact, realistic energy management). Keep the tone warm, concise, and naturally conversational. '
            ...
            .'The "tasks" array already reflects any filters from the student\'s request (for example: only certain courses, only exam-tagged items, school-only vs chores, or a specific time window like today or the next few days). You MUST treat it as the full universe of candidates and must not imagine tasks outside this array. You must not mention or compare to any tasks that are not present in the Context "tasks" array. If Context.filtering_summary.applied is true, explicitly mention that you filtered first and how many matching tasks remained before describing the ranking. '
            ...
            .'In recommended_action and reasoning, never mention internal database IDs such as "ID: 12" or numeric primary keys; use only human-readable titles and dates. Do not include IDs in JSON ranked lists. '
