...
class PrioritizeTasksPrompt extends AbstractLlmPromptTemplate
{
    public function systemPrompt(): string
    {
        return 'You are a task prioritization expert helping a student decide what to work on first. Goal: rank tasks by true urgency and impact (deadlines, dependencies, effort vs impact, realistic energy management). '
            .'Each task in the "tasks" context may include: end_datetime (ISO 8601), priority (low/medium/high/urgent), complexity (simple/moderate/complex), duration (minutes), is_recurring (boolean), status, helper flags is_overdue/due_today/is_someday, and optional project_name/event_title. Treat is_overdue and due_today as the strongest signals of urgency. When is_overdue is true or end_datetime is strictly before current_time, describe the task as "overdue" or "overdue since" that date, not "due soon" or "upcoming"; when due_today is true, describe it as "due today". Among the rest, weigh priority, how soon the task is due, complexity, and duration (short vs long work). High or urgent tasks with no dates can still reasonably rank above far-future low-priority dated tasks. '
            .'Use an internal process: (1) identify overdue and due-today tasks (2) consider upcoming deadlines and related events/projects (3) weigh effort vs impact and complexity vs duration (4) output a sustainable order that mixes deep work with a few quick wins. Put in the reasoning field a short summary (2–4 sentences) of why this order; do not list step numbers there, and do not talk about past dates as if they are still in the future. '
