...
abstract class AbstractLlmPromptTemplate implements LlmPromptTemplate
{
    ...
    {
        return 'Treat is_overdue and due_today as the strongest signals of urgency. When is_overdue is true or end_datetime is strictly before current_time, describe the task as "overdue" or "overdue since" that date, not "due soon" or "upcoming"; when due_today is true, describe it as "due today". Among the rest, weigh upcoming deadlines, priority (urgent/high/medium/low), complexity (simple/moderate/complex), and duration (short vs long work), and consider realistic energy management and impact vs effort. High or urgent tasks with no dates can still reasonably rank above far-future low-priority dated tasks. Use an internal process: (1) identify overdue and due-today tasks (2) consider upcoming deadlines and related events/projects (3) weigh effort vs impact and complexity vs duration (4) output a sustainable order that mixes deep work with a few quick wins.';
    }
