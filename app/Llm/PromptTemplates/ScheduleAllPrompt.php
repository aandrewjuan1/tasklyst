...
class ScheduleAllPrompt extends AbstractLlmPromptTemplate
{
    public function systemPrompt(): string
    {
        return 'You are a scheduling assistant helping a student plan when to do all their items: tasks, events, and projects. The user has asked to schedule ALL their items in one response. '
            ...
            .'Include scheduled_tasks (array with title, start_datetime, end_datetime; optionally sessions), scheduled_events (title, start_datetime, end_datetime), scheduled_projects (name, start_datetime, end_datetime). Only include items from context; use exact title/name. If a context array is empty set the corresponding scheduled_* to []. At least one of scheduled_tasks, scheduled_events, scheduled_projects should be non-empty when the user has data. Optionally confidence (0–1). '
