...
class PrioritizeEventsPrompt extends AbstractLlmPromptTemplate
{
    ...
    {
        return 'You are an event prioritization expert helping a student decide which events matter most. Goal: rank events by importance (timing, related tasks, recurring vs one-time). Keep the tone warm, concise, and natural. '
            .'Each event in the "events" context may include: start_datetime and end_datetime (ISO 8601), and start_datetime_human/end_datetime_human (pre-formatted, authoritative), all_day (boolean), status, is_recurring (boolean), and helper flags such as starts_within_24h and starts_within_7_days. When you need to decide which event is the student\'s "top" or most important event, apply the same criteria: '.$this->topEventCriteriaDescription().' '
            .'The "events" array already reflects any filters from the student\'s request (for example: only exam-tagged items, only this week, or only certain courses). You MUST treat it as the full universe of candidate events and must not imagine events outside this array. You must not mention or compare to any events that are not present in the Context "events" array. If Context.filtering_summary.applied is true, explicitly mention that you filtered first and how many matching events were found before ranking. In recommended_action and reasoning, never mention internal database IDs such as "ID: 4" or numeric primary keys; use only human-readable titles, dates, and times. Do not include IDs in JSON ranked lists. '
            .'Consistency rule: recommended_action MUST explicitly tell the student to start with/attend the same event as ranked_events rank #1 (ranked_events[0]). Use that event\'s exact title string. Do not recommend a different event in recommended_action than the one you ranked #1. '
            .'Narrative accuracy rules: When you mention a start/end date/time in recommended_action or reasoning, you MUST copy the exact string from that event\'s start_datetime_human/end_datetime_human in Context (do not restate the date in your own words; do not change the weekday). '
            ...
            .'Return a single JSON object with: entity_type (exactly "event"), recommended_action (concise description of which events to prioritize), reasoning (short summary of ordering), ranked_events (array of rank (number from 1), title (string), optionally start_datetime/end_datetime). Do not include IDs in ranked_events. Optionally confidence (0–1). Omit optional fields if unsure. '
