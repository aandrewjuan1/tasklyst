...
trait HandlesTasks
{
    ...
    {
        $user = $this->requireAuth(__('You must be logged in to restore tasks.'));
        if ($user === null) {
            return false;
        }

        $task = Task::query()->onlyTrashed()->forUser($user->id)->find($taskId);
        ...
        if ((int) $task->user_id !== (int) $user->id) {
            $this->dispatch('toast', type: 'error', message: __('Only the owner can restore this task.'));
        ...
        try {
            $restored = $this->restoreTaskAction->execute($task, $user);
        } catch (\Throwable $e) {
            ...
            $this->dispatch('toast', type: 'error', message: __('Couldn’t restore the task. Try again.'));
        ...
        $this->dispatch('toast', type: 'success', message: __('Restored the task.'));
    ...
    {
        $user = $this->requireAuth(__('You must be logged in to permanently delete tasks.'));
        if ($user === null) {
            return false;
        }

        $task = Task::query()->withTrashed()->forUser($user->id)->find($taskId);
        ...
        if ((int) $task->user_id !== (int) $user->id) {
            $this->dispatch('toast', type: 'error', message: __('Only the owner can permanently delete this task.'));
        ...
        try {
            $deleted = $this->forceDeleteTaskAction->execute($task, $user);
        } catch (\Throwable $e) {
            Log::error('Failed to permanently delete task.', [
                'user_id' => $user->id,
                'task_id' => $taskId,
                'exception' => $e,
            ]);

            $this->dispatch('toast', type: 'error', message: __('Couldn’t permanently delete the task. Try again.'));
        ...
        $this->dispatch('toast', type: 'success', message: __('Permanently deleted the task.'));
