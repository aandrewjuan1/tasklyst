...
<div
    ...
    x-data="{
        taskId: @js($taskId),
        currentEventId: @js($currentEventId),
        currentEventTitle: @js($currentEventTitle),
        open: false,
        ...
        loadErrorToast: @js(__('Could not load events. Please try again.')),
        ...
        async loadEvents() {
            ...
            try {
                const response = await $wire.$parent.$call('loadEventsForParentSelection', null, 50);
        ...
        removeTrashedEvent(eventId) {
            if (eventId == null) return;
            const id = Number(eventId);
            if (!Number.isFinite(id)) return;
            this.items = this.items.filter((item) => Number(item.id) !== id);
            if (Number(id) === Number(this.currentEventId)) {
                this.currentEventId = null;
                this.currentEventTitle = null;
            }
        },

        onEventTitleUpdated(detail) {
            if (!detail?.eventId || detail.title == null) return;
            const id = Number(detail.eventId);
            if (Number(id) === Number(this.currentEventId)) this.currentEventTitle = detail.title;
            const item = this.items?.find((i) => Number(i.id) === id);
            if (item) item.title = detail.title;
        },

        async selectEvent(item) {
            const eventId = item === null ? null : item.id;
            const eventTitle = item === null ? null : item.title;

            if (eventId != null && Number(eventId) === Number(this.currentEventId)) {
                this.close(this.$refs.trigger);
                return;
            }
            ...
            const snapshot = {
                ...
                eventTitle: this.currentEventTitle ?? null,
            };

            try {
                // PHASE 2: Optimistic update – dispatch so the card pill and parent's subtasks update (only event keys so we don't clear the project pill)
                window.dispatchEvent(new CustomEvent('workspace-task-parent-set', {
                    detail: {
                        taskId: this.taskId,
                        eventId,
                        eventTitle,
                        previousEventId: snapshot.eventId ?? undefined,
                    },
                    bubbles: true,
                }));
                if (eventId == null && snapshot.eventId != null) {
                    window.dispatchEvent(new CustomEvent('workspace-subtask-unbound', {
                        detail: {
                            taskId: this.taskId,
                            unboundProjectId: null,
                            unboundEventId: snapshot.eventId,
                        },
                        bubbles: true,
                    }));
                }
                ...
                const promise = $wire.$parent.$call('updateTaskProperty', this.taskId, 'eventId', eventId, true);
                ...
                window.dispatchEvent(new CustomEvent('workspace-task-parent-set', {
                    detail: {
                        ...
                        previousEventId: eventId ?? undefined,
                    },
                    bubbles: true,
                }));
    ...
    @workspace-event-trashed.window="removeTrashedEvent($event.detail.eventId)"
    @workspace-event-title-updated.window="onEventTitleUpdated($event.detail)"
    class="relative"
>
            ...
            <template x-if="!loading || items.length > 0">
                <div class="divide-y divide-border/60">
                    ...
                    >
                        ...
                        <span class="truncate text-muted-foreground/80">— {{ __('No event') }}</span>
                    </button>
                    ...
                    <template x-for="(item, index) in items" :key="item.id">
                        ...
                        >
                            <flux:icon name="calendar" class="size-4 shrink-0 text-brand-accent-violet/85 dark:text-violet-200/90" />
                            <span class="min-w-0 flex-1 truncate font-medium text-foreground" x-text="item.title"></span>
                        </button>
                    </template>
                </div>
            </template>
