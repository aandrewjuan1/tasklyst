                    ...
                    @foreach ($taskStatusOptions as $value => $label)
                        <label wire:key="fb-ts-{{ $value === '' ? 'all' : $value }}" class="workspace-filter-option" @click="closeFlyoutOnly()">
                            <input
                                ...
                                value="{{ $value }}"
                                @click='window.dispatchEvent(new CustomEvent("filter-optimistic", { detail: { key: "taskStatus", value: @json($value === '' ? null : $value) } } }))'
                            />
                    ...
                    @foreach ($taskPriorityOptions as $value => $label)
                        <label wire:key="fb-tp-{{ $value === '' ? 'all' : $value }}" class="workspace-filter-option" @click="closeFlyoutOnly()">
                            <input
                                ...
                                value="{{ $value }}"
                                @click='window.dispatchEvent(new CustomEvent("filter-optimistic", { detail: { key: "taskPriority", value: @json($value === '' ? null : $value) } } }))'
                            />
                            <span class="min-w-0 flex-1">{{ $label }}</span>
                        </label>
                    @endforeach
                    ...
                    @foreach ($taskComplexityOptions as $value => $label)
                        <label wire:key="fb-tc-{{ $value === '' ? 'all' : $value }}" class="workspace-filter-option" @click="closeFlyoutOnly()">
                            <input
                                ...
                                value="{{ $value }}"
                                @click='window.dispatchEvent(new CustomEvent("filter-optimistic", { detail: { key: "taskComplexity", value: @json($value === '' ? null : $value) } } }))'
                            />
                            <span class="min-w-0 flex-1">{{ $label }}</span>
                        </label>
                    @endforeach
                        ...
                        @foreach ($eventStatusOptions as $value => $label)
                            <label wire:key="fb-es-{{ $value === '' ? 'all' : $value }}" class="workspace-filter-option" @click="closeFlyoutOnly()">
                                <input
                                    ...
                                    value="{{ $value }}"
                                    @click='window.dispatchEvent(new CustomEvent("filter-optimistic", { detail: { key: "eventStatus", value: @json($value === '' ? null : $value) } } }))'
                                />
                                <span class="min-w-0 flex-1">{{ $label }}</span>
                            </label>
                        @endforeach
                    ...
                    @foreach ($recurringOptions as $value => $label)
                        <label wire:key="fb-rec-{{ $value === '' ? 'all' : $value }}" class="workspace-filter-option" @click="closeFlyoutOnly()">
                            <input
                                ...
                                value="{{ $value }}"
                                @click='window.dispatchEvent(new CustomEvent("filter-optimistic", { detail: { key: "recurring", value: @json($value === '' ? null : $value) } } }))'
                            />
                            <span class="min-w-0 flex-1">{{ $label }}</span>
                        </label>
                    @endforeach
